...
import { fixturePerfectBonus } from "./quiz_bonus";
import type { QuizPaper, QuizResult } from "./quiz";

describe("quiz bonus HTTP", () => {
  ...
  it("serves bonus paper and grades", async () => {
    const paperRes = await fetch(`${base}/api/quiz/bonus`);
    ...
    const miss = await fetch(`${base}/api/quiz/bonus/check`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: "b1",
        answer: { choice: "Одне поле balance=150 і нотатка «50 бонус»" },
      }),
    });
    ...
    const hit = await fetch(`${base}/api/quiz/bonus/grade`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ answers: fixturePerfectBonus() }),
    });
