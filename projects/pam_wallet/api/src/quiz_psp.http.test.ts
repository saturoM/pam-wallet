...
import { fixturePerfectPsp } from "./quiz_psp";
import type { QuizPaper, QuizResult } from "./quiz";

describe("quiz psp HTTP", () => {
  ...
  it("serves psp paper and grades", async () => {
    const paperRes = await fetch(`${base}/api/quiz/psp`);
    ...
    const miss = await fetch(`${base}/api/quiz/psp/check`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: "p1", answer: { choice: "Так — банк уже підтвердив людину" } }),
    });
    const missBody = (await miss.json()) as { ok: boolean; hint?: string };
    assert.equal(missBody.ok, false);
    assert.ok(missBody.hint);

    const hit = await fetch(`${base}/api/quiz/psp/grade`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ answers: fixturePerfectPsp() }),
    });
