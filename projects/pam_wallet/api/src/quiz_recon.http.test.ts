...
import { fixturePerfectRecon } from "./quiz_recon";
import type { QuizPaper, QuizResult } from "./quiz";

describe("quiz recon HTTP", () => {
  ...
  it("serves recon paper and grades", async () => {
    const paperRes = await fetch(`${base}/api/quiz/recon`);
    ...
    const miss = await fetch(`${base}/api/quiz/recon/check`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: "r1", answer: { choice: "Так, зелений касир = день закритий" } }),
    });
    const missBody = (await miss.json()) as { ok: boolean; hint?: string };
    assert.equal(missBody.ok, false);
    assert.ok(missBody.hint);

    const hit = await fetch(`${base}/api/quiz/recon/grade`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ answers: fixturePerfectRecon() }),
    });
